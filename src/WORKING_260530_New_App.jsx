...
export default function App() {
  ...
  const pushUndo = (snapshot) => {
    setUndoStack((prev) => [snapshot.map((cue) => ({ ...cue })), ...prev].slice(0, 10));
  };
  ...
  const loadProject = async (e) => {
    ...
    reader.onload = async () => {
      try {
        ...
        const pdfFile = new File([pdfBuffer], "project.pdf", {
        ...
        setColors({ ...colors, ...(project.colors || {}) });
        setStyles({ ...styles, ...(project.styles || {}) });
      } catch (err) {
    ...
    <div style={{ padding: 20 }}>
      ...
      >
        ...
        <label>
          ...
          <input
            type="file"
            accept=".cueproj,application/json"
        ...
        <span style={{ fontSize: 12, opacity: 0.75 }}>
          Hotkeys: S/1, T/2, D/3, N/4, W/5, M/6 · Undo: Ctrl/Cmd+Z · Edit: double-click cue
        </span>
